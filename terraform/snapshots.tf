...
resource "yandex_compute_snapshot_schedule" "instance_group_disks" {
  ...
  snapshot_count   = 3
  retention_period = "72h"  # 3 дня
  
  # Диски из instance group (если нужно)
  # Примечание: В Yandex Cloud нет прямой поддержки снапшотов дисков instance group через Terraform
  # Это можно сделать через API или скрипт
